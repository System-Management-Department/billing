...
class Merchandise{
	...
	*/
	...
	public static function execInsert($db, $q, $context, $result){
		$db->beginTransaction();
		try{
			$insertQuery = $db->insertSet("merchandises", [
			],[
				"created" => "now()",
				"modified" => "now()",
			]);
		...
		if(!$result->hasError()){
			...
			@Logger::record($db, "登録", ["merchandises" => $q["code"]]);
		}
	}
	...
	public static function execUpdate($db, $q, $context, $result){
		...
		$db->beginTransaction();
		try{
			$updateQuery = $db->updateSet("merchandises", [
			],[
				"modified" => "now()",
			]);
		...
		if(!$result->hasError()){
			$result->addMessage("編集保存が完了しました。", "INFO", "");
			@Logger::record($db, "編集", ["merchandises" => $code]);
		}
	}
	...
	public static function execDelete($db, $q, $context, $result){
		...
		if(!$result->hasError()){
			...
			@Logger::record($db, "削除", ["merchandises" => $q["code"]]);
		}
	}
}