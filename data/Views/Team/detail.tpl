		...
		.then(response => {
			if(response.success){
				...
				for(let message of response.messages){
					Flow.DB.insertSet("messages", {title: "チーム削除", message: message[0], type: message[1], name: message[2]}, {}).apply();
				}
	...
	<div class="container border border-secondary rounded p-4 mb-5 bg-white">
		<div class="row gap-4 align-items-start">
			<table class="col table">
				...
				<tr>
					<th scope="row" class="bg-light align-middle ps-4">
						<label class="form-label ls-1" for="name-input">チーム名</label>
					</th>
					<td>
				...
				<tr>
					<th scope="row" class="bg-light align-middle ps-4">
						<label class="form-label ls-1" for="kana-input">チーム名カナ</label>
					</th>
					<td>
