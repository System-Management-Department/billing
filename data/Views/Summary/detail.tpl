		...
		.then(response => {
			if(response.success){
				...
				for(let message of response.messages){
					Flow.DB.insertSet("messages", {title: "摘要削除", message: message[0], type: message[1], name: message[2]}, {}).apply();
				}
			...
			<table class="table w-50">
				<tr>
					<th scope="row" class="bg-light align-middle ps-4">
						<label class="form-label ls-1" for="code-input">摘要コード</label>
					</th>
					<td>
				...
				<tr>
					<th scope="row" class="bg-light align-middle ps-4">
						<label class="form-label ls-1" for="type-input">摘要種別</label>
					</th>
					<td>
						<div class="col-10">{$data.type|summaryType}</div>
					</td>
				</tr>
				<tr>
					<th scope="row" class="bg-light align-middle ps-4">
						<label class="form-label ls-1" for="name-input">摘要名</label>
					</th>
					<td>
