...
new VirtualPage("/", class{
	constructor(vp){
		...
		const query = transaction.select("ALL")
			.addTable("purchase_relations")
			.andWhere("purchase_relations.pu IS NOT NULL")
		...
		const fields = [
			"STRFTIME('%Y-%m',purchase_workflow.regist_datetime)",
			"sales_slips.project",
			"STRFTIME('%Y-%m-%d %H:%M',sales_workflow.regist_datetime)",
			"clients.name",
			"sales_slips.subject",
			"suppliers.name",
			"purchases.amount_inc",
			"purchases.amount_exc",
			"STRFTIME('%Y-%m-%d',purchases.payment_date)"
		];
		...
		if(memover){
			...
			tfoot.innerHTML = `<td colspan="${fields.length + 1}" class="text-start text-danger">エラー　出力可能な請求一覧表の上限を超過しています。</td>`;
			alert("出力可能な請求一覧表の上限を超過しています。");
		}else{
...
Promise.all([
	...
	try{
		const data = cache.select("ONE").setTable("purchase_data").setField("selected").andWhere("slip_number=?", search.key).apply();
		const formData = new FormData();
		formData.append("pu_array", data);
		return fetch("/Purchase/search", {
			method: "POST",
			body: formData
			...
			<template data-page="/">
				<div slot="main" style="background-color: gray; padding:10px;" class="d-flex flex-column gap-2 overflow-auto">
					<print-page size="A3" orientation="l" class="flex-shrink-0">
						<div class="d-flex justify-content-between"><div style="font-size: 20px;">【 仕入一覧表 】</div><div>日付<div class="d-inline-block text-end" style="width: 6em;"><span data-slot="year"></span>年</div><div class="d-inline-block text-end" style="width: 3em;"><span data-slot="month"></span>月</div><div class="d-inline-block text-end" style="width: 3em;"><span data-slot="date"></span>日</div></div></div>
