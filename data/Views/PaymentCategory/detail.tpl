{block name="title"}入金区分詳細画面{/block}
		...
		.then(response => {
			if(response.success){
				...
				for(let message of response.messages){
					Flow.DB.insertSet("messages", {title: "入金区分削除", message: message[0], type: message[1], name: message[2]}, {}).apply();
				}
	...
	<div class="container border border-secondary rounded p-4 mb-5 bg-white">
		<div class="row">
			<table class="table w-50">
				...
				<tr>
					<th scope="row" class="bg-light align-middle ps-4">
						<label class="form-label ls-1" for="type-input">入金種別</label>
					</th>
					<td>
						<div class="col-10">{$data.type|paymentType}</div>
					</td>
				</tr>
				<tr>
					<th scope="row" class="bg-light align-middle ps-4">
						<label class="form-label ls-1" for="name-input">入金区分名</label>
					</th>
					<td>
