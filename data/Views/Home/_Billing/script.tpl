{literal}
	new VirtualPage("/Billing", class{
		...
		constructor(vp){
			...
			document.querySelector('table-sticky').columns = dataTableQuery("/Billing#list").apply().map(row => { return {label: row.label, width: row.width, slot: row.slot}; });
		...
		reload(){
			...
			setDataTable(document.querySelector('table-sticky'), dataTableQuery("/Billing#list").apply(), data);
		}
	});
{/literal}