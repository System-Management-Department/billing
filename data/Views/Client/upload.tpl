	...
	*[Symbol.iterator](){
		...
		reader.addEventListener("load", e => {
			...
			this.response.createTable("csv", [
				"code","name","kana","short_name","location_zip","location_address1","location_address2","location_address3","phone","fax","email","homepage",
				"transactee","transactee_honorific","department","managerial_position","tax_round","tax_processing","close_processing","close_date",
			...
			if(res instanceof FormData){
				...
				if(response.success){
					...
					for(let message of response.messages){
						Flow.DB.insertSet("messages", {title: "得意先CSV取込", message: message[0], type: message[1], name: message[2]}, {}).apply();
					}
...
<form action="{url action="import"}" method="POST">
	<div class="container border border-secondary rounded p-4 bg-white table-responsive">
		<table class="table table_sticky_list text-nowrap">
			...
			<tbody id="list">{predefine name="listItem" constructor=["closeProcessing", "prefectures", "invoiceFormat", "taxRound", "taxProcessing", "closeDate", "monthList", "unitPriceType", "existence"] assign=["obj", "dataValidation"]}
