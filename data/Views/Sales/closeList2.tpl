	...
	constructor(vp){
		...
		try{
			...
			for(let {key, label} of csvFormats){
				...
				const file = serializer.serializeToString(csvData[key]);
				const blob = SJISEncoder.createBlob(file, {type: "text/csv"})
			...
			opener.IPCPromise.billingClose(csvCloseData).then(() => {});
