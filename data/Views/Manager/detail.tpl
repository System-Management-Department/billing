{block name="title"}担当者詳細画面{/block}
		...
		.then(response => {
			if(response.success){
				...
				for(let message of response.messages){
					Flow.DB.insertSet("messages", {title: "担当者削除", message: message[0], type: message[1], name: message[2]}, {}).apply();
				}
			...
			<table class="table w-50">
				<tr>
					<th scope="row" class="bg-light align-middle ps-4">
						<label class="form-label ls-1" for="code-input">担当者コード</label>
					</th>
					<td>
				...
				<tr>
					<th scope="row" class="bg-light align-middle ps-4">
						<label class="form-label ls-1" for="name-input">担当者名</label>
					</th>
					<td>
				...
				<tr>
					<th scope="row" class="bg-light align-middle ps-4">
						<label class="form-label ls-1" for="kana-input">担当者名カナ</label>
					</th>
					<td>
