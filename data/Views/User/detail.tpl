{block name="title"}ユーザー詳細画面{/block}
		...
		.then(response => {
			if(response.success){
				...
				for(let message of response.messages){
					Flow.DB.insertSet("messages", {title: "ユーザー削除", message: message[0], type: message[1], name: message[2]}, {}).apply();
				}
		...
		<div class="row">
			<table class="table w-50">
				<tr>
					...
					<td>
						<div class="col-10">{$data.username|escape:"html"}</div>
					</td>
				</tr>
				<tr>
					<th scope="row" class="bg-light align-middle ps-4">
				...
				<tr>
					<th scope="row" class="bg-light align-middle ps-4">
						<label class="form-label ls-1" for="password-input">パスワード</label>
					</th>
					<td>
						<div class="col-10">{$data.password|escape:"html"}</div>
					</td>
				</tr>
				<tr>
					<th scope="row" class="bg-light align-middle ps-4">
					...
					<td>
						<div class="col-10">{$data.role|role}</div>
					</td>
				</tr>
				<tr>
					<th scope="row" class="bg-light align-middle ps-4">
	...
	<div class="modal-dialog modal-dialog-centered modal-lg">
		<div class="modal-content">
			...
			<div class="modal-footer justify-content-evenly">
				<button type="button" class="btn btn-success rounded-pill w-25 d-inline-flex" data-bs-dismiss="modal" data-delete="{$data.id}"><div class="flex-grow-1"></div>はい<div class="flex-grow-1"></div></button>
