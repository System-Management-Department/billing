    ...
    var jexcel = (function(el, options) {
        ...
        var defaults = {
            ...
            text: Object.assign({
                ...
                showingPage: 'ページ {0}/{1} を表示中',
                show: '表示 ',
                search: '検索',
                columnName: '列名',
                renameThisColumn: 'この列名を変更',
                orderAscending: '昇順で並べ替え',
                orderDescending: '降順で並べ替え',
                insertANewRowBefore: '新しい行を前に挿入',
                insertANewRowAfter: '新しい行を後ろに挿入',
                ...
                thisActionWillClearYourSearchResultsAreYouSure: 'この操作は検索結果をクリアします。よろしいですか？',
        ...
        obj.createTable = function() {
            ...
            tempCol.textContent = obj.options.text.rowNumber
            tempCol.style.textAlign = obj.options.colHeaderAlign;
            ...
            for (j = 0; j < max_j; j++) {
                for (i = 0; i < max_i; i++) {
                    if (obj.options.data[j] == undefined) {
                        obj.options.data[j] = (obj.options.dataProxy == null) ? [] : obj.options.dataProxy();
                    }
        ...
        */
        ...
        obj.createCellHeader = function(colNumber) {
            ...
            var colAlign = obj.options.colHeaderAlign;
        ...
        obj.createToolbar = function(toolbar) {
            ...
            obj.toolbar.appendChild(toolbar);
        }
            ...
            if (obj.records[y][x].classList.contains('readonly') == true && ! force) {
                ...
                if (obj.options.columns[x].editor) {
                    ...
                    if (obj.options.columns[x].type == 'checkbox' || obj.options.columns[x].type == 'radio') {
                        ...
                        // Update data
                        obj.options.data[y][x] = (value == 1 || value == true || value == 'true' || value == 'TRUE');
                    } else if (obj.options.columns[x].type == 'dropdown' || obj.options.columns[x].type == 'autocomplete') {
                        // Update data
                        obj.options.data[y][x] = value;
                    } else if (obj.options.columns[x].type == 'calendar') {
                ...
                obj.updateRow(x, y, value, record);
            }

            return record;
        }
        
        obj.updateRow = function(x, y, value, record) {
            // Update cell
            for (let j = 0; j < obj.records[y].length; j++) {
                if (obj.options.columns[j].type == 'checkbox' || obj.options.columns[j].type == 'radio') {
                    ...
                    // Try formatted date
                    var formatted = null;
                    if (! validDate(obj.options.data[y][j])) {
                        var tmp = jSuites.calendar.extractDateFromString(obj.options.data[y][j], obj.options.columns[j].options.format);
                        if (tmp) {
                            formatted = tmp;
                        }
                    }
                    obj.records[y][j].textContent = jSuites.calendar.getDateString(formatted ? formatted : obj.options.data[y][j], obj.options.columns[j].options.format);
                } else if (obj.options.columns[j].type == 'color') {
                    // Render
                    if (obj.options.columns[j].render == 'square') {
                        var color = document.createElement('div');
                        color.className = 'color';
                        color.style.backgroundColor = obj.options.data[y][j];
                        obj.records[y][j].textContent = '';
                        obj.records[y][j].appendChild(color);
                    } else {
                        obj.records[y][j].style.color = obj.options.data[y][j];
                        obj.records[y][j].textContent = obj.options.data[y][j];
                    }
                } else if (obj.options.columns[j].type == 'image') {
                    obj.records[y][j].innerHTML = '';
                    if (obj.options.data[y][j] && obj.options.data[y][j].substr(0, 10) == 'data:image') {
                        var img = document.createElement('img');
                        img.src = obj.options.data[y][j];
                        obj.records[y][j].appendChild(img);
                    }
                } else if(obj.options.data[y][j] == null){
                    obj.records[y][j].innerHTML = "";
                } else {
                    // Label
                    if (obj.options.columns[j].type == 'html') {
                        obj.records[y][j].innerHTML = stripScript(obj.parseValue(j, y, obj.options.data[y][j]));
                    } else {
                        if (obj.options.stripHTML === false || obj.options.columns[j].stripHTML === false) {
                            obj.records[y][j].innerHTML = stripScript(obj.parseValue(j, y, obj.options.data[y][j], obj.records[y][j]));
                        } else {
                            obj.records[y][j].textContent = obj.parseValue(j, y, obj.options.data[y][j], obj.records[y][j]);
                        }
                    }
                    // Handle big text inside a cell
                    if (obj.options.columns[j].wordWrap != false && (obj.options.wordWrap == true || obj.options.columns[j].wordWrap == true || obj.records[y][j].innerHTML.length > 200)) {
                        obj.records[y][j].style.whiteSpace = 'pre-wrap';
                    } else {
                        obj.records[y][j].style.whiteSpace = '';
                    }
                }
                
                // Overflow
                if (j > 0) {
                    if (obj.options.data[y][j]) {
                        obj.records[y][j-1].style.overflow = 'hidden';
                    } else {
                        obj.records[y][j-1].style.overflow = '';
                    }
                }
            }
            ...
            // On change
            if (x == null) {
                obj.dispatch('onchange', el, null, null, y, null, null);
            } else {
        ...
        obj.insertRow = function(mixed, rowNumber, insertBefore) {
            ...
            if (obj.options.allowInsertRow == true) {
                ...
                for (var row = rowIndex; row < (numOfRows + rowIndex); row++) {
                    ...
                    obj.options.data[row] = (obj.options.dataProxy == null) ? [] : obj.options.dataProxy();
